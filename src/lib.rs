...
pub fn decode_easy(input: &str, delimiter: &str) -> Vec<u8> {
    decode(input.split(delimiter))
}

pub fn decode<I, S>(input: I) -> Vec<u8>
    where
        S: AsRef<str>,
        I: Iterator<Item=S>,
{
...
pub fn encode_easy(input: &mut impl Read, delimiter: &str) -> String {
    encode(input).join(delimiter)
}

pub fn encode(input: &mut impl Read) -> Vec<&'static str> {
...
mod tests {
    ...
    use crate::{decode_easy, encode_easy, shift_11, write_with_shift_11};
